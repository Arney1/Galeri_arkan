...
function Home({ auth, username, sendNotif }) {
  // useEffect(() => {
  ...
  useEffect(() => {
    const fetchPhotos = async () => {
      const limitz = { limit, username };
      const response = await fetch("http://localhost:8081/api/main/", {
        credentials: "include",
        method: "POST",
  ...
  return (
    <>
      <Helmet>
        <title>Galeri | Home</title>
      </Helmet>
      <div className="flex flex-col gap-2 p-2 w-full">
        <h1 className="text-3xl">New posts</h1>
